...
class ActionCableConnector extends BaseActionCableConnector {
  ...
  onInternalChatMessageCreated = data => {
    ...
    const channel = this.app.$store.getters['internalChat/getChannelById'](data.internal_chat_channel_id);
    if (channel) {
      const currentUserId = this.app.$store.getters.getCurrentUser?.id;
      const isOwnMessage = data.sender?.id === currentUserId;
      const activeChannelId = this.app.$store.getters['internalChat/getActiveChannelId'];
      ...
      this.app.$store.dispatch('internalChat/updateChannel', {
        ...
        unread_count: isActiveChannel || isOwnMessage ? channel.unread_count || 0 : (channel.unread_count || 0) + 1,
        has_unread_mention: isActiveChannel || isOwnMessage ? false : channel.has_unread_mention || isMentioned,
  ...
  onInternalChatTypingOn = ({ channel, user }) => {
    this.app.$store.dispatch('internalChatTypingStatus/create', { channelId: channel.id, user });
  };

  onInternalChatTypingOff = ({ channel, user }) => {
    this.app.$store.dispatch('internalChatTypingStatus/destroy', { channelId: channel.id, user });
  };
  ...
  onInternalChatChannelUpdated = data => {
    ...
    if (memberIds && currentUserId && data.channel_type === 'private_channel') {
      ...
      const existing = this.app.$store.getters['internalChat/getChannelById'](data.id);
      if (!existing) {
